	...
	<div class="portlet-configuration-body-content">
		...
		<aui:form action="<%= updateRolePermissionsURL.toString() %>" cssClass="form" method="post" name="fm">
			...
			<div class="container-fluid-1280">
				...
				List<String> excludedRoleNamesList = new ArrayList<>();

					excludedRoleNamesList.add(RoleConstants.ADMINISTRATOR);

				if (filterGroupRoles) {
					excludedRoleNamesList.add(RoleConstants.ORGANIZATION_ADMINISTRATOR);

					excludedRoleNamesList.add(RoleConstants.ORGANIZATION_OWNER);

					excludedRoleNamesList.add(RoleConstants.SITE_ADMINISTRATOR);

					excludedRoleNamesList.add(RoleConstants.SITE_OWNER);
				}

				if (filterGuestRole) {
					excludedRoleNamesList.add(RoleConstants.GUEST);
				}

				String[] excludedRoleNamesArray = new String[excludedRoleNamesList.size()];

				for (int i = 0; i < excludedRoleNamesList.size(); i++) {
					excludedRoleNamesArray[i] = excludedRoleNamesList.get(i);
				}
				...
				int count = RoleLocalServiceUtil.getGroupRolesAndTeamRolesCount(company.getCompanyId(), searchTerms.getKeywords(), excludedRoleNamesArray, roleTypes, modelResourceRoleId, teamGroupId);

				roleSearchContainer.setTotal(count);

				List<Role> roles = RoleLocalServiceUtil.getGroupRolesAndTeamRoles(company.getCompanyId(), searchTerms.getKeywords(), excludedRoleNamesArray, roleTypes, modelResourceRoleId, teamGroupId, roleSearchContainer.getStart(), roleSearchContainer.getResultEnd());
